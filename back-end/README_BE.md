...
| ---------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **POST /reports/upload**                        | {title, description?, category_id, latitude, longitude, is_public, photos? } | Upload a new report with optional photos. Requires to be logged in as a citizen.                                                       |
...
| **PATCH /reports/report/:id/status**            | { status, status_reason? }                                                   | Updates the status of a report (e.g., REJECTED). Requires Admin/Municipality privileges.                                               |
| **GET /reports/search-reports**                 | `page_num?`, `page_size?`, `status?`, `is_public?`, `category_id?`           | Searches reports with optional filters: status, visibility, category. <br> Results are paginated and ordered by `updatedAt` descending |
| **GET /reports/assigned-to-techOfficer**        |                                                                              | Retrieves the list of reports assinged to a specific technical officer.                                                                |
| **GET /reports/tos-users**                      | `category_id` (required)                                                     | Retrieves all municipality users (TOS) responsible for a specific report category.                                                     |
| **PATCH /reports/report/:id/assign**            | { assigned_to }                                                              | Assigns a report to a specific municipality user (Technical Officer). Sets status to 'Assigned'.                                       |
| **PATCH /reports/report/:id/assign-maintainer** | { maintainer_id }                                                            | Assigns a report to an external maintainer. Requires Technical Officer role.                                                           |
| **GET /reports/maintainer-users**               |                                                                              | Retrieves all users with the 'external_maintainer' role.                                                                               |
| **GET /reports/assigned-to-maintainer**         |                                                                              | Retrieves all the reports currently assigned to a specific external maintainer                                                         |
