...
| **GET /report/get-map-reports**                | {statusArray?} | Fetches all the reports stored on the database that match one of the statuses contained inside statusArray (if it's empty simply returns all reports). <br> Result is just a regular Report Array, ordered by `updatedAt` descending |
