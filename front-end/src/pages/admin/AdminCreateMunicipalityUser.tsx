    ...
    return (
        <div className="min-h-screen bg-gray-50 py-10">
            <div className="max-w-3xl mx-auto px-4">
                <h1 className="text-2xl font-bold mb-6">Create Municipality User</h1>

                <Card className="p-6">
                    {errors.submit && <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded mb-6">{errors.submit}</div>}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {rolesError && <div className="text-sm text-red-600 mb-2">{rolesError}</div>} {/* Display roles fetching error */}

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {roles.map(r => (
                                    <label key={r.id} className="flex items-center space-x-2 bg-white p-2 rounded border">
                                        <input
                                            type="checkbox"
                        ...
                        <div className="flex items-center justify-between">
                            <Button type="submit" disabled={loading} style={{ backgroundColor: '#5199CD' }}>
                                {loading ? 'Creating...' : 'Create Municipality User'}
                            </Button>
                            <Link to="/admin" className="text-sm text-gray-600">Back to Admin</Link>
                        </div>
                    </form>
                </Card>
            </div>
